...
import { createBrowserInstance, closeBrowserInstance, createNewPage, closePage } from './pageAndBrowser.js'
import { getResponses, getRequests, getCookies, getLocalStorage, determineCookieBannerState, addRequestToDb} from './crawlMeasurements.js'
...
async function takeMeasurements (page, browser, websiteUrl, connection, requestData, wordCorpus, parentCutoff, childrenCutoff) {
...
async function evaluateWebsite (browser, websiteUrl, connection, wordCorpus, parentCutoff, childrenCutoff) {
  ...
  if (returnCode === 'Success') {
    LOADED_COUNTER++
    await takeMeasurements(page, browser, websiteUrl, connection, requestData, wordCorpus, parentCutoff, childrenCutoff)
  } else {
    successBool = false
  }
...
async function crawlMain (browser, resultPath, urlList, connection, device,
  ...
  for (const websiteUrl of urlList) {
    ...
    try {
      const [evaluationResult] = await Promise.race([
        await evaluateWebsite(browser, websiteUrl, connection, wordCorpus, parentCutoff, childrenCutoff),
