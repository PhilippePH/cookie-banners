...
async function randomiseURLs (path) {
  ...
  const firstThousand = myURLs.slice(0, 1000)

  const rest = myURLs.slice(1000)
  const thirdLength = rest.length / 3
  ...
  const file = createWriteStream('webCrawler/shuffled.txt')
